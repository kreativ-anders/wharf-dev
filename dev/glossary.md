...
| **Share** | Make a running project reachable from other devices on the local network — a phone — at `http(s)://<address>:<port>`, a port of its own recorded as `projects[].lan_port`. Until the user stops sharing, stops the project or quits; never at a start | `features/sharing.feature` |
| **Network certificate** (authority) | Wharf's own certificate authority for projects shared over HTTPS, separate from mkcert's: installed once on a phone, may sign only for private addresses, lasts a year. *Replace* deletes its key | `features/sharing.feature`, `daemon/internal/lan` |
