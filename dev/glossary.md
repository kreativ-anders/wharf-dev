...
| **Share** | Make a running project reachable from other devices on the local network — a phone — at `http(s)://<address>:<port>`, a port of its own recorded as `projects[].lan_port`. Until the project stops or Wharf quits; never at a start | `features/sharing.feature` |
