...
// on the front door port 80, and 443 while it uses SSL; its loopback port
// behind it. nginx takes both front-door ports in one server block, Apache
// needs a virtual host per port. The template's header comment is left out;
// once per file is enough, and it is in the template
